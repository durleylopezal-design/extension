...
js/components/*.js     → modal, toast, placeholder ("módulo en construcción"), charts (wrappers Chart.js: initIngresos/initSegmentos específicos del Dashboard + initBar/initLine/initDoughnut genéricos), sidebar, topbar, y el patrón de listado compartido por CRM: dataTable, filterPanel, listShell (los orquesta), recordForm (panel deslizante de creación), kanbanBoard, emptyState
js/data/*.js           → arrays de ejemplo, uno por dominio (eventos, financiero, certificados, consolidado, cuentas, informes, paneles, leads, contactos, oportunidades, documentos)
